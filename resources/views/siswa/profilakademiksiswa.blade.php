...
@section('data_header')
    <div class="col-md-9">
        <h2 class="fw-bold mb-1">{{ $user->userable->nama }}</h2>
        <p class="text-white-50 mb-2"><i class="fas fa-id-card me-2"></i>{{ $user->userable->nis_nisn }}
        </p>
        <p class="text-white-50"><i class="fas fa-envelope me-2"></i>{{ $user->email }}</p>
        <p class="text-white-50">
            <i class="fas fa-birthday-cake me-2"></i>{{ \Carbon\Carbon::parse($user->userable->tanggal_lahir)->age }}
            tahun
        </p>
        <div class="mt-3">
            <span class="badge bg-light text-primary me-2 p-2">
                <i class="fas fa-chalkboard-teacher me-1"></i> {{ Str::title($user->role) }}
            </span>
            <span class="badge bg-light text-primary p-2">
                <i class="fas fa-user-check me-1"></i> {{ Str::title($user->userable->status) }}
            </span>
        </div>
...
@section('biodata')
    @include('siswa.profilakademiksiswa.biodata_siswa')
@endsection

@section('data_akun')
    @include('siswa.profilakademiksiswa.data_akun_siswa')
@endsection
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-body">
                <form action="{{route('profilakademiksiswa.UpdateProfileSiswa')}}" method="POST" enctype="multipart/form-data" id="photoForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="text-center mb-4">
                        <div class="position-relative d-inline-block">
                            ...
                            {{-- Image --}}
                            <img id="previewImg"
                                src="{{ asset('storage/profile_siswa/' . (Auth::user()->userable->foto_profile ?? 'default-profile.jpg')) }}"
                    ...
                    <div class="mb-3">
                        <label for="profil" class="form-label fw-bold"><i class="fas fa-file-image me-2"></i>Pilih
                            Foto</label>
                        <input type="file" class="form-control" name="profil_siswa" id="profil" accept="image/*">
                    </div>
